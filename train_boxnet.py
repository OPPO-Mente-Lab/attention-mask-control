...
from utils.model_utils import add_module_args, configure_optimizers
...
from input_process import PositionNet
...
class StableDiffusion(LightningModule):
    ...
    def add_module_specific_args(parent_parser):
        ...
        parser.add_argument('--timestep_range', type=int,
                            default=[0, 1000], nargs="+")
        parser.add_argument('--mean_embed', action='store_true', default=False)
    ...
    def __init__(self, args):
        ...
        self.boxnet, self.boxnet_criterion, self.postprocessors = build_model(args)
        if args.boxnet_path is not None:
            self.boxnet.load_state_dict(torch.load(args.boxnet_path))
        self.position_net = PositionNet(in_dim=768, out_dim=768)
        if args.pnet_path is not None:
            self.position_net.load_state_dict(torch.load(args.pnet_path))
        self.save_hyperparameters(args)

        for _, param in self.text_encoder.named_parameters():
            param.requires_grad = False
        for _, param in self.position_net.named_parameters():
            param.requires_grad = False
    ...
    def configure_optimizers(self):
        ...
        for name, p in self.boxnet.named_parameters():
            model_params.append(p)
            trainable_names.append(name)
        print("trainable_names: {}".format(trainable_names))
    ...
    def log_imgs(
        ...
        **kwargs,
        ...
        for data in inputs:
            ...
            if args.mean_embed:
                # 这里拿了pooled_output(N,768)[1]
                tmp_embed = self.text_encoder(cat_input_id)[0]
                tmp_embed = torch.mean(tmp_embed, dim=1)
            else:
        ...
        uncond_cat_embeddings = torch.zeros_like(cat_embeddings)
        input_objects = torch.cat(
            [uncond_cat_embeddings, cat_embeddings])  # 2B * 30 * 768
        # input_objects = torch.cat([cat_embeddings]*2) # 2B * 30 * 768
        ...
        if args.fuser_type is None:
            text_embeddings = torch.cat(
                [text_embeddings, objects], dim=1)  # B*(77+30)*768
            objects = None
        ...
        for i, t in enumerate(tqdm(timesteps)):
            ...
            noise_pred = self.unet(
                latent_model_input, t, encoder_hidden_states=text_embeddings, objs=input_objects).sample
            # perform guidance
            if do_classifier_free_guidance:
            ...
            step_images.append([(out_images[k], t.item())
                               for k in range(len(out_images))])
            ################################################################
    ...
    def get_bbox_input(self, batch):
        ...
        for i, length in enumerate(batch["bbox_length"]):  # batch loop
            ...
            if args.mean_embed:
                tmp_embed = self.text_encoder(cat_input_id)[0]
                tmp_embed = torch.mean(tmp_embed, dim=1)  # 尝试取mean()
            else:
                tmp_embed = self.text_encoder(cat_input_id)[1]
    ...
    def training_step(self, batch, batch_idx):
        ...
        sqrt_alpha_prod = self.noise_scheduler.alphas_cumprod[timesteps].to(
            latents.device) ** 0.5
        ...
        cat_embeddings, masks = self.get_bbox_input(batch)
        objects = self.position_net(
            batch["bboxes"], cat_embeddings, masks)  # B*N*768
        if args.fuser_type is None:
            encoder_hidden_states = torch.cat(
                [encoder_hidden_states, objects], dim=1)  # B*(77+30)*768
            objects = None
        ...
        noise_pred = self.unet(noisy_latents, timesteps,
                               encoder_hidden_states, objs=objects).sample
        ...
        outputs = self.boxnet(features, noise_level, queries=cat_embeddings)
        loss_dict = self.boxnet_criterion(
            outputs, batch['targets'])  # , out_prob=out_prob
        weight_dict = self.boxnet_criterion.weight_dict
        bbox_loss = sum(loss_dict[k] * weight_dict[k]
                        for k in loss_dict.keys() if k in weight_dict)

        lr = self.trainer.optimizers[0].param_groups[0]["lr"]

        diffusion_loss = F.mse_loss(
            noise_pred, noise, reduction="none").mean([1, 2, 3]).mean()
        loss = bbox_loss  # + bbox_loss * args.loss_proportion
        self.log("diffusion_loss", diffusion_loss.item(),
                 on_epoch=False, prog_bar=True, logger=True)

        self.log("train_loss", loss.item(), on_epoch=False,
        ...
        if self.trainer.global_rank == 0:
            if (self.global_step+1) % args.save_steps == 0:
                ...
                model_path = os.path.join(save_path, f"boxnet.pth")
                ...
                with torch.no_grad():
                    try:
                        ...
                        print(
                            f"No prompts read from file: {args.test_prompts}, skip test.")
                    else:
                        print(inputs)
                        images, step_images = self.log_imgs(
                            latents.device, inputs, num_images_per_prompt=1)
                        img_path = os.path.join(save_path, "test_images")
                        for i, img in enumerate(images):
                            res_path = os.path.join(img_path, "{}".format(i))
                            os.makedirs(res_path, exist_ok=True)
                            img.save(os.path.join(res_path, f"result.png"))
                            for cur_step_image in step_images:
                                step_image, t = cur_step_image[i]
                                step_image.save(os.path.join(
                                    res_path, f"step_{t}.png"))
                                save_colored_mask(os.path.join(
                                    res_path, f"color_step_{t}.png"), step_image)
    ...
    def on_train_epoch_end(self):
        if self.trainer.global_rank == 0:
            print('saving model...')
            save_path = os.path.join(
                args.default_root_dir, f'hf_out_{self.trainer.current_epoch}.pt')
...
if __name__ == '__main__':
    ...
    assert len(
        args.timestep_range) >= 2, "Must input timestep_range with (start, end)"
    ...
    def collate_fn(examples):
        ...
        for example in examples:
            ...
            if bbox_num > args.num_queries:
                ...
                # bbox = torch.zeros((args.num_queries, 4))
        ...
        label_values = label_values.to(
            memory_format=torch.contiguous_format).int()
